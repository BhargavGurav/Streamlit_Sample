...
st.header('2. Interactive Line Plot')
st.subheader('2.1 Line chart')
df = pd.read_csv('lang_data.csv')
lang_list = df.columns.tolist()
lang_choices = st.multiselect('Chose your language ', lang_list)
...
st.subheader('3.2 Pie chart with mulitple parameter')
fig = px.pie(df, values = 'total_bill', names='day', opacity= .6, color_discrete_sequence = px.colors.sequential.RdBu)
st.plotly_chart(fig)
